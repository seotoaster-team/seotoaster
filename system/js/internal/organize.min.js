...
$(this).prop("checked"))})});
function saveCategoriesOrder(){var a=0,b=[];$("#sortable-main").find(".category-data").each(function(){$(this).find(".ranking").html("#"+ ++a);b.push($(this).attr("id"));$(this).find(".organise li").each(function(){b.push($(this).attr("id"))})});$.ajax({type:"post",url:$("#website_url").val()+"backend/backend_page/organize/",data:{act:"save",ordered:JSON.stringify(b)},dataType:"json",beforeSend:function(){showSpinner()},success:function(a){hideSpinner();showMessage(a.responseText,a.error)}})}
