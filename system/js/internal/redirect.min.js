...
$(document).ready(function(){$("#to-url").chosen({search_contains:!0});$("#frm-redirects-search").submit(function(a){a.preventDefault();a=$("#redirect-search").val();showSpinner();$.ajax({url:$("#website_url").val()+"backend/backend_seo/loadredirectslist/searchName/"+a,type:"post",dataType:"json",success:function(a){hideSpinner();$("#redirects-box").find("#redirects-list").html(a.redirectsList)}})});$(document).on("click",".clear-btn",function(){$("#redirect-search").val("");$("#frm-redirects-search").trigger("submit")});
$("#redirects-box").append($(".paginator"));var b=$(".paginator").length,a=$(".not-mutch-paginator").length;(1<b||1==a)&&$(".paginator:last").remove()});function reloadRedirectsList(){$("input[name='redirect-search']").text("");var b=$("#redirect-search").val();showSpinner();$.getJSON($("#website_url").val()+"backend/backend_seo/loadredirectslist/searchName/"+b,function(a){hideSpinner();$("#redirects-list").html(a.redirectsList);checkboxRadioStyle()})};
