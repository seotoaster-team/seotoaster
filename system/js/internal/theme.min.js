...
$("#frm_template").find("#template-type").val(a.responseText.type),$.getJSON($("#website_url").val()+"backend/backend_theme/pagesviatemplate/",{template:a.responseText.name},function(a){$("#pcount").text(a.pagesUsingTemplate)}),showTemplateList(),showListPages(),hideSpinner())},"json");$("#templatelist").removeClass("show")}).on("click",".template_delete",function(){deleteTemplate($(this).closest(".template_item"));return!1});$(document).on("click","#listpages-btn",function(a){a.preventDefault();
$("#listpages").addClass("show")}).on("click",".closebutton .hide, .show~#frm_template",function(a){$("#templatelist, #listpages").removeClass("show");return!1});$(document).on("click","#listtemplates-btn",function(a){a.preventDefault();a=$("#templatelist");a.addClass("show").find(".content").accordion("refresh");a.find(".template_group").css({"max-height":a.height()-a.find(".header").outerHeight(!0)-(a.find(".content").outerHeight(!0)-a.find(".content").height())-a.find(".template_header:last").outerHeight(!0)*
a.find(".template_header").length})}).on("click",".closebutton .hide, .show~#frm_template",function(a){$("#templatelist, #listpages").removeClass("show");return!1});$("textarea").on("keydown",function(a){a.ctrlKey&&83==a.keyCode&&(a.preventDefault(),saveTemplate())})});
function showTemplateList(c){var b=$("#templatelist");b.find(".content").length&&"update"!=c||$.post($("#website_url").val()+"backend/backend_theme/gettemplate/",{listtemplates:"all",pageId:$("#pageId").val()},function(a){b.html(a).find(".content").accordion({heightStyle:"content",header:".template_header",collapsible:!0});a=$("#template_id").val();$(".template_item").removeClass("curr-template").find(".template-check").remove();$(".template_item").has('input[value="'+a+'"]').addClass("curr-template").find(".template_name").before('<span class="template-check icon-check icon16"/></span>')},
"html")}function showListPages(c){$.post($("#website_url").val()+"backend/backend_page/listpages/",{template:$("#template_id").val(),format:"html"},function(b){$("#listpages").html(b)},"html")}
