...
function toggleExternalLink(){var a=$("#external-link-status"),c="h1 header-title meta-keywords meta-description page-options curr-template".split(" "),d=$("#external-link-switch").prop("checked")?1:0;a.length&&(a.val(d),$("#toggle-optimized").is(":checked")||($.each(c,function(b,a){$("#"+a).prop("readonly",d).toggleClass("noedit")}),d?($("#additional-navigation-block").addClass("hidden"),$("#additional-navigation-block").find("checkbox").prop("checked",!1),$("#additional-navigation-block").find("#page-target-blank").prop("checked",
!0),$(".template-section").css({visibility:"hidden"}),$("#url").val($("#external-link").val()),""===$.trim($("#url").val())&&$("#url").prop("placeholder","Your link to an external website")):($("#additional-navigation-block").removeClass("hidden"),$("#url").val($("#original-page-url").val()),$(".template-section").css({visibility:"visible"}),$("#url").prop("placeholder",""))))}function datepickerCallback(){$("#publish-at").val($(this).val())}
...
b.show().next(".menu-info").hide()}}$(document).on("change","#template-categories",function(){var a=$(this).val();$(".template_group").hide();$("#"+a).show()});$(document).on("change","#page-target-blank",function(){$(this).is(":checked")&&$("#page-not-clickable").prop("checked",!1)});$(document).on("change","#page-not-clickable",function(){$(this).is(":checked")&&$("#page-target-blank").prop("checked",!1)});
function showTemplatesList(){var a=$("#templatelist");$.post($("#website_url").val()+"backend/backend_theme/gettemplate/pageId/"+$("#pageId").val(),{listtemplates:"all",beforeSend:showSpinner("#templatelist")},function(c){a.html(c);hideSpinner()})}
