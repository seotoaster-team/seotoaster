$(function(){$("#pageCategory").hide();$("#templateId").prev().hide();var a=[$("#header-title"),$("#url"),$("#nav-name")],d=$("input.menu-selector"),e=!$("#pageId").val(),c=$("#pageCategory").val();$("#optimized").val($("#toggle-optimized").attr("checked")?1:0);$(document).on("click",".template_name",function(){var b=$(this).parent().find('input[name="template-id"]').val();$("#templateId").val(b);$("#curr-template").text(b);$("#templatelist").removeClass("show")}).on("click","input.menu-selector",
function(b){checkMenu($(b.currentTarget).attr("id"),c)}).on("click","#toggle-optimized",function(){var b=$(this),a=b.is(":checked")?1:0;a?toggleOptimized(a):showConfirm("Are you sure? You will lose experts optimization once you save your changes !",function(){toggleOptimized(a)},function(){b.prop("checked",!0)})}).on("click","#published",function(){var a=$("#draft");a.length&&a.val($(this).prop("checked")?0:1)}).on("blur","#datepicker",function(){$("#publish-at").val($(this).val())});e?($("#h1").keyup(function(){var b=
$(this).val();$(a).each(function(){$(this).val(b)})}),d.each(function(){$(this).attr("checked",!1)})):checkMenu()});
