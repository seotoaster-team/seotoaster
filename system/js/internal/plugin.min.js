$(function(){$("#plugintab").tabs({active:0,beforeLoad:function(b,a){a.panel.addClass("plugins-list h425");a.ajaxSettings.dataFilter=function(b){a.panel.html($.parseJSON(b).pluginsList);$(".plugin-item a.plugin-control, .plugin-item a.plugin-endis").button()};a.jqXHR.done(function(){})}});$(document).on("click",".plugin-control",function(){triggerPlugin("install",$(this))}).on("click",".plugin-endis",function(){triggerPlugin("onoff",$(this))}).on("click",".readme-plugin",function(){var b=$(this).data("name");
...
function pluginCallback(){$.getJSON($("#website_url").val()+"backend/backend_plugin/list/",function(b){$(".plugins-list").html(b.pluginsList);$("a.plugin-control, a.plugin-endis").button()})}
