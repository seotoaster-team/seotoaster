...
$(this);if(a.is(":checkbox")||a.is(":radio")||a.is(":hidden"))if(a.parent().is("label"))a.parent().tooltip("destroy").removeAttr("title","").removeClass("notvalid");else{var b=a.prop("id");$(document).find('[for="'+b+'"]').tooltip("destroy").removeAttr("title","").removeClass("notvalid")}else a.tooltip("destroy").removeClass("notvalid").removeAttr("title");a.unbind()}),showTooltip(".notvalid","error","right")):e&&g&&a.dialog?smoke.confirm(a.responseText,function(a){a&&"undefined"!=typeof g&&null!=
...
function showTooltip(a,b,c){$(a+"[title]").tooltip();var d="",f="";switch(c){case "right":d="left+10 center";f="right center";break;case "left":d="right-10 center";f="left center";break;case "top":d="center bottom-10";f="center top";break;case "bottom":d="center top+10",f="center bottom"}$(a+"[title]").tooltip("option",{tooltipClass:b,position:{my:d,at:f,using:function(a,b){$(this).css(a);$("<span>").addClass("arrow").addClass(b.vertical).addClass(b.horizontal).appendTo(this)}}})}
...
function showMailMessageEdit(a,b){$.getJSON($("#website_url").val()+"backend/backend_config/mailmessage/",{trigger:a},function(a){$(e).remove();var d=a.responseText.message,f=a.responseText.dialogTitle,h=a.responseText.dialogOkay,f=0<f.length?f:"Edit mail message before sending",h=0<h.length?h:"Okay",d=d?a.responseText.message:"success",e=$('<div class="msg-edit-screen"></div>').append($('<textarea id="trigger-msg" rows="10"></textarea>').val(d).css({resizable:"none"}));$("#trigger-msg").val(d);e.dialog({modal:!0,
title:f,width:600,resizable:!1,show:"clip",hide:"clip",draggable:!1,buttons:[{text:h,click:function(a){e.dialog("close");b($("#trigger-msg").val())}}]})},"json")};
