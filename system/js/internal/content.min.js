...
($(".mce-toolbar-grp").show(),$(".above-editor-links").removeClass("grid_4").addClass("grid_8"),$("#tabs").removeClass("grid_8").addClass("grid_4"))});$(document).on("click",'[aria-label="Fullscreen"]',function(){$(window.parent.document).find('[aria-describedby="toasterPopupDraggable"]').toggleClass("screen-expand");var a=$("#tabs"),c=a.height(),b=a.find(".ui-tabs-nav").height(),d=a.find("#adminthingsviewer .ui-accordion-header"),e=d.length,d=d.outerHeight(),f=a.find("#adminselectimgfolder").outerHeight(),
...
error:function(){a.html("Unable to load files list")}})});$("#widgets").click(function(){var a=$("#widgets_maker");a.html().length||$.ajax({url:$("#website_url").val()+"backend/backend_content/loadwidgetmaker",type:"post",dataType:"json",success:function(b){a.html(b.responseText)},error:function(){a.html('<h4 style="text-align: center;">Unable to load widget maker.</h4>')}})});var b=localStorage.getItem(generateStorageKey()),e=$(this).find("#content").val();null!==b&&b!=e&&showConfirm("We have found content that has not been saved! Restore?",
function(){tinymce.activeEditor.setContent(b);$("#content").val(b)},function(){localStorage.removeItem(generateStorageKey())},"success");$(document).on("click","span.refresh-items",function(a){a.preventDefault();$("#adminselectimgfolder").trigger("change")})});function dispatchEditorKeyup(b,e,a){var c=a;null===c&&(c=setTimeout(function(){localStorage.setItem(generateStorageKey(),tinymce.activeEditor.getContent());c=null},1E3))}
