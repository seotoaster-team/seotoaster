...
class Application_Model_DbTable_Page extends Zend_Db_Table_Abstract {
    ...
    public function fetchAllMenu($menuType, $fetchSysPages = false) {
        ...
        $select   = $this->getAdapter()->select()
            ->from('page', array('id', 'parentId' => 'parent_id', 'protected', 'external_link_status', 'external_link', 'page_folder', 'is_folder_index', 'exclude_category', 'page_target_blank', 'not_clickable'))
    ...
    public function findByUrl($pageUrl = Helpers_Action_Website::DEFAULT_PAGE) {
        ...
        $select     = $this->_getOptimizedSelect(false, array('id', 'parent_id', 'template_id', 'last_update', 'silo_id', 'protected', 'system', 'news', 'preview_image', 'original_url' => 'page.url', 'page_folder', 'is_folder_index', 'exclude_category', 'publish_at'));
